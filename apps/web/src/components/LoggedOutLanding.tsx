...
export function LoggedOutLanding({ title, description, onLogin }: Props) {
  return (
    <>
      <section className="logged-out-hero">
        <div className="logged-out-hero__inner">
          <h1 className="logged-out-hero__title">{title}</h1>
          <p className="logged-out-hero__desc">{description}</p>
          <div className="hero-mirofish__cta-primary">
          ...
          <div className="hero-mirofish__cta-note">
            <GitHubAuthNote />
          </div>
        </div>
      </section>
      <LandingFeatures onLogin={onLogin} compact />
    </>
  );
}
