...
const links = [
  { to: '/', label: 'Overview', end: true, icon: 'grid' as const },
  { to: '/explore', label: 'Explore', end: false, icon: 'search' as const },
  { to: '/repos', label: 'My PRs', end: false, icon: 'repo' as const },
  { to: '/journey', label: 'Journey', end: false, icon: 'timeline' as const },
];
      ...
      <aside className="sidebar">
        <div className="border-b border-[var(--color-border)] px-4 py-5">
          <div className="relative z-10 flex items-center gap-3">
            ...
            <div>
              <p className="text-sm font-semibold leading-tight tracking-tight">OSCT</p>
              <p className="text-[10px] text-[var(--color-muted)]">Contribution Tracker</p>
            </div>
          </div>
        </div>

        <nav className="sidebar__nav flex flex-col gap-1 p-2.5">
            ...
            <NavLink
              ...
              className={({ isActive }) =>
                [
                  ...
                  isActive
                    ? 'nav-link-active font-medium text-[var(--color-text)]'
